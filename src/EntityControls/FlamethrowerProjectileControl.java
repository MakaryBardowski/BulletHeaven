 ...
 * @author 48793
 */
public class FlamethrowerProjectileControl extends AbstractControl implements Savable, Cloneable {
    ...
    public FlamethrowerProjectileControl(Projectile owner, GameState gs, Vector3f destination, Particle[] particles) {
        this.owner = owner;
        this.gs = gs;
    ...
    protected void controlUpdate(float tpf) {
        if (spatial != null && owner.getNode().getWorldTranslation().getY() >= 18 * tpf) {
            ...
            ultimateMoveVec = new Vector3f(owner.getNode().getLocalRotation().getRotationColumn(2).getX() * speed * tpf, owner.getNode().getLocalRotation().getRotationColumn(2).getY() * speed * tpf, owner.getNode().getLocalRotation().getRotationColumn(2).getZ() * speed * tpf);
...
//                Node node = (Node)gs.getAssetManager().loadModel("Models/c4/c4.j3o");
//                gs.getDebugNode().attachChild(node);
//                node.move(owner.getNode().getWorldTranslation().clone());
//                
                owner.hit(gs);
                owner.getNode().removeControl(this);
                ...
                travelledDistance += ultimateMoveVec.length();
...
//                    LightingMethods.terminateLightSource(owner.getNode().getWorldTranslation(), maxRange, gs);
    ...
    public Control cloneForSpatial(Spatial spatial) {
        final FlamethrowerProjectileControl control = new FlamethrowerProjectileControl(owner, gs, new Vector3f(0, 0, 0), particles);
    ...
    private void stopParticles() {
        ...
        for (Particle p : particles) {
            if (p != null) {
                p.velocity.set(0, p.velocity.getY() / 5 + 4 * RANDOM.nextFloat(), 0);
            }
        }

    }

    private void slowParticles(int val) {

        for (Particle p : particles) {
            if (p != null) {
                p.velocity.set(p.velocity.getX() / val, p.velocity.getY() / 5 + 4 * RANDOM.nextFloat(), p.velocity.getZ() / val);
            }
        }
