 ...
 * @author 48793
 */
public class Shotgun extends RangedWeapon implements ItemInterface, LeftHandEquippableItem, RightHandEquippableItem {

    private int pelletNumber = 5;
    private int pelletSpread;

    public Shotgun(Node node, String name, String iconPath, String texturePath, String dropPath, String dropTexturePath, String mobPath, String mobTexturePath, boolean droppable, float damage, float attackSpeed, float accuracyPenalty, boolean showMuzzleFlash, int muzzleFlashTime, int currentAmmo, int maxAmmo) {
    ...
    public void playerEquipItemR(Player p, GameState gs) {
        ...
        Node shotgunModel = (Node) p.getAssetManager().loadModel(getName());
        shotgunModel.scale(0.7f);
        p.setRightHandNode(shotgunModel); // set right hand model as this weapons model
    ...
    private void playerShoot(Player p, GameState gs, CameraAndMouseControl c, EntityManager em, Node nodeToCheckCollisionOn, Node debugNode) {
        if (getCurrentAmmo() > 0) {
            ...
            for (int i = 0; i < pelletNumber; i++) {
                pelletShot(p, gs, c, em, nodeToCheckCollisionOn, debugNode);
            }
    ...
    public void pelletShot(Player p, GameState gs, CameraAndMouseControl c, EntityManager em, Node nodeToCheckCollisionOn, Node debugNode) {
...
//                     shotDirection.setY(  (float)Math.sin(Math.toDegrees(angle))*shotDirection.getX() + (float) Math.cos(Math.toDegrees(angle))*shotDirection.getY());
        ...
        if (results.size() > 0) {
            ...
            if (enemyHit != null) {
                ...
                gs.getPlayer().dealDamage((MobInterface) enemyHit, (( gs.getPlayer().getEquippedRightHand()).getDamage() / pelletNumber * dmgMultiplier), gs, em,headName);
...
//                       Sphere sphere1 = new Sphere(30, 30, 0.05f);
...
//                mark1.move(closest.getContactPoint());
    ...
    public void reload(Player p, GameState gs){
         ...
         p.getRightHandAnimChannel().setAnim("Reload");
            p.getRightHandAnimChannel().setSpeed(2); // modifies shooting speed
            p.getRightHandAnimChannel().setLoopMode(LoopMode.DontLoop);
