 ...
 * @author 48793
 */
public class MinimapMobIndicatorControl extends AbstractControl implements Savable, Cloneable {
    ...
    public MinimapMobIndicatorControl(Node owner,Mob p,float GRID_SIZE,float movementRatio) {
        this.owner = owner;
        this.p = p;
 ...
 if(spatial != null && !p.isDead() ){
         owner.setLocalTranslation((int) p.getNode().getWorldTranslation().getZ()/GRID_SIZE*movementRatio,(int) p.getNode().getWorldTranslation().getX()/GRID_SIZE*movementRatio,1);
 }else{
 owner.removeFromParent();
 p.getNode().removeControl(this);
 }
    }

    @Override
    public Control cloneForSpatial(Spatial spatial) {
        final MinimapMobIndicatorControl control = new MinimapMobIndicatorControl(owner,p,GRID_SIZE,movementRatio);
